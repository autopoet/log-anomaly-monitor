...
1. 通过 Docker Compose 启动 RabbitMQ，或者使用 `docker compose up --build` 一次性启动所有服务。
...
为了在没有 Docker 或 RabbitMQ 的环境下也能快速查看页面效果，项目另外提供了 `web.preview` 入口。该入口会在 Web 服务内部生成模拟的设备分析数据，并通过与正式模式相同的 WebSocket 消息格式推送给前端。

预览模式只用于展示页面效果和截图，不替代完整实验链路。课程要求中的 MOM 消息队列功能仍由 RabbitMQ、`collector` 和 `analyzer` 模块完成。
